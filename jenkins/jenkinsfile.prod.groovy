...
DB_BUILD = common.DB_NAME + "-build-" + common.web_environments.prod.tag
DB_IMAGESTREAM_NAME = common.DB_NAME + "-" + common.web_environments.prod.tag
...
stage("Deploy" + WEB_IMAGESTREAM_NAME + "to ${common.web_environments.prod.name}") {
  ...
  node{
    try{
      ...
      WEB_IMAGE_HASH = common.getLaprodHash(WEB_IMAGESTREAM_NAME, environment)          
...
stage("Deploy to" + API_NAME + "${common.api_environments.prod.name}") {
  ...
  node{
    try{
      // Tag the images for deployment based on the image's hash
      API_IMAGE_HASH = common.getLaprodHash(API_IMAGESTREAM_NAME, environment)          
