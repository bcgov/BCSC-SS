...
describe('BC Keycloak Login with BC services Card', function()
{
    it('Visit the BC Dev Page and Login', function(){
        ...
        cy.get('#csn').type('SS4BPS214').should('have.value', 'SS4BPS214')
        cy.wait(5000)
        cy.log("Click on Continue to go to next page")
        cy.get('#continue').click()
        cy.wait(5000)
        cy.log("Enter Passcode")
        cy.get('#passcode').type('98914',{ log: false })
        cy.wait(5000)
        cy.log("Click Submit")
        ...
        cy.get('input[data-test-id="input-profile-email"]').type('SS4BPS214@gov.bc.ca').should('have.value', 'SS4BPS214@gov.bc.ca')
        cy.wait(5000)
