...
def test_meta_no_commit_hash(client):
    ...
    rv_ = client.get('/api/v1/meta/info')

    assert rv_.status_code == 200
    assert rv_.json == {'API': f'selfservice_api/{__version__}'}


def test_meta_with_commit_hash(monkeypatch, client):
    """Assert that the endpoint return __version__ \
    and the last git hash used to build the services image."""
    from selfservice_api.version import __version__
    ...
    rv_ = client.get('/api/v1/meta/info')
    assert rv_.status_code == 200
    assert rv_.json == {'API': f'selfservice_api/{__version__}-{commit_hash}'}
